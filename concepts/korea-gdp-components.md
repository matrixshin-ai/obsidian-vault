---
title: 한국 GDP 지출 구성
category: concepts
tags: [한국경제, gdp, 소비, 투자, 수출, 내수]
aliases: [GDP 구성, 지출접근법, Korea GDP Components]
relationships:
  - target: "[[concepts/korea-gdp]]"
    type: derived_from
  - target: "[[concepts/korea-economic-growth]]"
    type: related_to
sources:
  - "Economy/1 GDP/1) GDP 일반/2 통계와 동향.md"
  - "Economy/1 GDP/2) 소비/소비.md"
  - "Economy/1 GDP/3) 투자/투자 개요.md"
  - "Economy/1 GDP/4) 수출/수출 개요와 동향.md"
summary: 2024년 한국 GDP는 소비50%·투자30%·정부15%·수출45%·수입40% 구성으로, 소비 비중이 OECD 평균(62.9%)을 크게 하회해 내수 취약성을 드러낸다.
provenance:
  extracted: 0.80
  inferred: 0.18
  ambiguous: 0.02
base_confidence: 0.47
lifecycle: draft
lifecycle_changed: "2026-05-18"
created: 2026-05-18T00:00:00+09:00
updated: 2026-05-19T00:00:00+09:00
---

# 한국 GDP 지출 구성

GDP = C(소비) + I(투자) + G(정부지출) + X(수출) − M(수입) 공식에 따른 2024년 한국의 지출 구성이다.

## 2024년 구성 비율

| 항목 | 한국 | OECD 평균 | 비고 |
|---|---|---|---|
| C 소비 | 50% | 62.9% | **OECD 하회 → 내수 취약** |
| I 투자 | 30% | 23.2% | OECD 상회, 최근 약화 중 |
| G 정부지출 | 15% | — | — |
| X 수출 | 45% | — | 고수출 의존 구조 |
| M 수입 | 40% | — | — |

*비교 대상: Europe C=63%, I=23%*

## 주요 시사점

- 한국은 경제 성장의 절반 이상을 소비가 아닌 수출·투자에 의존하고 있어 내수 기반이 취약하다.
- 소비의 세부 구성: 상품 소비 40%, 서비스 소비 60%로 추정된다.
- 투자 비중은 OECD 평균을 웃돌지만 최근 약화 추세이다.
- 고수출 의존 구조는 글로벌 경기 변동에 대한 취약성을 높인다. ^[inferred]

## 내수 취약성 구조

소비 비중이 낮은 배경으로는 [[concepts/korea-economic-growth]]에서 확인할 수 있는 성장 둔화와 더불어, 과도한 가계부채(GDP 대비 91.7%), 고령화에 따른 소비 성향 저하 등이 복합적으로 작용한다. ^[inferred]

## 관련 문서

- [[concepts/korea-gdp]] — 전체 GDP 규모와 세계 위상
- [[concepts/korea-economic-growth]] — 연도별 실질성장률 추세
- [[concepts/korea-consumption]] — 소비 구조와 위기 (가계부채·고령화·고금리)
- [[concepts/korea-investment]] — 투자 동향 (건설투자 급락, 해외 자본 유출)
- [[concepts/korea-exports]] — 수출 구조 (반도체·미중 의존, 관세 리스크)

## Open Questions

- 투자(I) 약화가 지속될 경우 GDP 성장 동력은 어디서 확보하는가?
- 서비스 소비 비중 60%는 선진국 평균 대비 어느 수준인가? ^[ambiguous]
- 이재명 정부 출범 전 '대기 상태' 경제([[synthesis/korea-economy-pre-lee-government]])가 각 GDP 구성 항목에 미친 구체적 충격은?

## Sources

- `Economy/1 GDP/1) GDP 일반/2 통계와 동향.md`
